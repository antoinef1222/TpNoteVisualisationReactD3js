...
class ScatterplotD3 {
    ...
    updateAxis(visData, xAttribute, yAttribute){
        ...
        // Mise à jour de l'intitulé des labels
        this.xLabel = xAttribute;
        this.svg.select(".xAxisLabel")
            .text(this.xLabel)
        ;

        this.yLabel = yAttribute;
    ...
    renderScatterplot(visData, xAttribute, yAttribute, controllerMethods){
            ...
            .join(
                enter => {
                    ...
                    itemG.append("circle")
                        ...
                        .attr("stroke", "red")
                    ;
                    this.updateMarkers(itemG, this.xLabel, this.yLabel);
                },
                update => {
                    this.updateMarkers(update, this.xLabel, this.yLabel);
                },
                exit => {
                    exit.remove();
                }
        ...
        /**
         * Mise à jour de la sélection des points via la brush
         * @param {*} event Evènement sur la brush et ses dimensions
         */
        const updateSelection = (event) => {

            /**
             * Récupère la liste des points qui se trouve dans la brush
             * Pour cela, on récupère l'évènement déplacement de la brush, on scale la position des points, puis on les compare pour déterminer leur appartenance
             */
            const selectedItems = this.svg.selectAll(".markerG")
                .filter((point) => {
                    const cx = this.xScale(point[this.xLabel]);
                    const cy = this.yScale(point[this.yLabel]);
                    return this.isBrushed(event.selection, cx, cy);
                })
                .data();
            ;

            // On renvoie au store les nouveaux objets sélectionnés pour mettre à jour l'application globale de la sélection
            controllerMethods.handleSelection(selectedItems);
        }


        // Définition de la brush
        const brush = d3.brush()
            .extent([[0, 0], [this.width, this.height]])
            .on("start brush", updateSelection)
        ;
        
        // Ajout de la brush dans le graphe
